...
sim.simFam = function(nSibsPatern, nSibsMatern, nSibs, nGrandchild, 
                      ...
                      includeGeno = FALSE, includeGrandparents = TRUE, censoring = TRUE, 
                      genderPro = NULL, genoMat = NULL,
                      CurAge = NULL, affTime = FALSE, 
                      BiomarkerTesting = NULL, includeBiomarkers = FALSE) {
        ...
        if (all(c("BRCA1", "BRCA2") %in% genes)) {
          ...
          if (sum(fam$isAffBC == 1) > 0) {
            fam[fam$isAffBC == 1, names(dim(BiomarkerTesting$Breast))[1:5]] =
              t(apply(fam[fam$isAffBC == 1,c("BRCA1", "BRCA2")], 1, function(x) {
                probs = BiomarkerTesting$Breast[-1, -1, -1, -1, -1,
                                                as.character(x["BRCA1"]),
                                                as.character(x["BRCA2"])]
          ...
          warning("Need to include both BRCA1 and BRCA2 to simulate breast cancer markers.")
        }
      }  
      if ("Colorectal" %in% cancers) {
        if (all(c("MLH1", "MSH2", "MSH6", "PMS2") %in% genes)) {
          fam$MSI = NA
          if (sum(fam$isAffCOL == 1) > 0) {
            fam$MSI[fam$isAffCOL == 1] = apply(fam[fam$isAffCOL == 1,c("MLH1", "MSH2", "MSH6", "PMS2")], 1, function(x) {
          ...
          warning("Need to include all four of MLH1, MSH2, MSH6 and PMS2 to simulate colorectal cancer markers.")
        }
      } 
