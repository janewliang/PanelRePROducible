...
# Summary of differences between PanelPRO and BayesMendel carrier probabilities
carrier_summary = t(sapply(carrier_probs, function(x){
  summary(x$carrier_PP_sub[x$keep_idx] - x$carrier_BM_sub[x$keep_idx])
}))
save(carrier_summary, file="results/diagnostics/carrier_summary.rData")
