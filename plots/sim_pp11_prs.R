...
all_dirs = c("../sim/pp11/", "../sim/pp11_prs_snp20/", "../sim/pp11_prs_snp40/")
...
# Forest plots
png(paste0(img_dir, "diagnostics_PRS.png"), width=600, height=420)
...
legend("bottom", inset=c(0,-6), 
       legend=c("No PRS", "20-SNP PRS", "40-SNP PRS"), 
...
lapply(names(metrics), function(x) {
  tab = data.frame(as.vector(sapply(names(genes), function(y){ c(y, "", "")})),
                   c("No PRS", "20-SNP PRS", "40-SNP PRS"), 
