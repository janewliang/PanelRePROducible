...
img_dir = "img/sim_upper_pp5/"

# List of all directories to search in
all_dirs = c("../hcp/pp5/", "../sim/pp5/", "../big_fam/pp5/")
...
# Mutations
mutations = names(diagnostics_list[[1]])
names(mutations) = gsub("_", " ", mutations)


# Pull out and reshape relevant diagnostics
reshaped_diagnostics = lapply(metrics, function(x){
  rbind(sapply(mutations, function(y){ diagnostics_list[[1]][[y]][x,mods] }), 
        sapply(mutations, function(y){ diagnostics_list[[2]][[y]][x,mods] }), 
        sapply(mutations, function(y){ diagnostics_list[[3]][[y]][x,mods] }))
})
...
my_pch = c(16, 2, 4)
my_pch_mat = matrix(rep(my_pch, times=length(mutations)), ncol=length(my_pch), byrow=TRUE)
...
# Forest plots
png(paste0(img_dir, "diagnostics.png"), width=600, height=250)
...
lapply(names(metrics), function(x) {
  ...
  names(tab) = c("Mutation", "Data", "Estimate", 
