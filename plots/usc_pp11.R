...
res_dir = "../usc/pp11/"
res_rm_dir = "../usc/pp11_riskmod/"
img_dir = "img/usc_pp11/"
...
plot.new()
par(bg=NA, xpd=TRUE)
legend("bottomleft", inset=c(0.2,-6), 
       legend=c("Full Model", "Submodel"), 
       fill=my_cols, bty="n", ncol=2, cex=1.4)
legend("bottomright", inset=c(0,-6), 
...
png(paste0(img_dir, "perc_improve_box.png"), width=400, height=600)
layout(mat = matrix(c(1, 2, 3), 
                    nrow = 3, ncol = 1),
...
invisible(sapply(names(metrics), function(x) {
  ...
  if (x == "MSE") {
    ...
    text(x=1:length(BM_genes), y = -6.25, 
...
# Load carrier probabilities
load(paste0(res_dir, "results/diagnostics/carrier_probs.rData"))
# Set up data frame for plotting
carrier_df = data.frame(
  brcapro_any = carrier_probs$BRCAPRO_genes$carrier_PP_sub, 
  MMRpro_any = carrier_probs$MMRpro_genes$carrier_PP_sub, 
  PanelPRO_any = carrier_probs$Any$carrier_PP, 
  sapply(carrier_probs, function(x) {
    x$mutation_status
  })
)
# Colors for plotting
carrier_df$gene_group = "black"
carrier_df$gene_group[carrier_df$BRCAPRO_genes == 1] = "#0072B2"
carrier_df$gene_group[carrier_df$MMRpro_genes == 1] = "#E69F00"

# Plot carrier probabilities for BRCAPRO and PanelPRO-11
png(paste0(img_dir, "thresh_any_brcapro.png"), width = 350, height = 350)
par(mar = c(4, 4, 1, 1), bg=NA)
plot(carrier_df$brcapro_any[carrier_df$Any==1], 
     carrier_df$PanelPRO_any[carrier_df$Any==1], 
     pch = 16, 
     col = carrier_df$gene_group[carrier_df$Any==1], 
     xlab = "Any BRCAPRO", ylab = "Any PanelPRO-11")
abline(h=0.025)
abline(v=0.025)
legend("top", legend = c("BRCA1 or BRCA2", "MLH1, MSH2, or MSH6", 
                         "Other Pathogenic Variant"), 
       pch = 16, col = c("#0072B2", "#E69F00", "black"), bty = "n")
dev.off()

# Frequency table
table("Any BRCAPRO" = carrier_df$brcapro_any[carrier_df$Any==1] > 0.025, 
      "Any PanelPRO-11" = carrier_df$PanelPRO_any[carrier_df$Any==1] > 0.025)


# Plot carrier probabilities for MMRpro and PanelPRO-11
png(paste0(img_dir, "thresh_any_mmrpro.png"), width = 350, height = 350)
par(mar = c(4, 4, 1, 1), bg=NA)
